...
export function blockHeightToRewardSats(blockHeight: number) {
  return (210_000n * 100_000_000n) / 2n ** BigInt(blockHeightToPeriodId(blockHeight))
}

export function scrollIntoView(element: HTMLElement | SVGElement | null, offset: number = 0) {
  if (!element) return
...
export const fetcher = (...args: any[]) => {
  ...
  if (Array.isArray(args[0])) return fetch(args[0][0], args[0][1]).then((res) => res.json())
  // @ts-ignore
  return fetch(...args).then((res) => res.json())
}
export const textFetcher = (...args: any[]) => {
  // if first arg is array, consider the second arg as options
  if (Array.isArray(args[0])) return fetch(args[0][0], args[0][1]).then((res) => res.text())
  // @ts-ignore
